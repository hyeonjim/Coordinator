...
public class TestCodeGenerationService {
    ...
    private String callAiGenerateTestCode(String fileName, String code) {
        ...
        try {
            // RestTemplate 생성 및 UTF-8 인코딩 설정
            RestTemplate restTemplate = new RestTemplate();
            restTemplate.getMessageConverters().add(0, new StringHttpMessageConverter(StandardCharsets.UTF_8));

            // HTTP 헤더 설정 (text/plain)
            ...
            // AI 서비스 호출
            String url = aiBaseUrl + "/generate-testcode?fileName=" + fileName;
    ...
    public AiReport generateReport(String runOutput) {
        try {
            ...
            // 요청 본문 생성
            HttpEntity<String> request = new HttpEntity<>(runOutput, headers);

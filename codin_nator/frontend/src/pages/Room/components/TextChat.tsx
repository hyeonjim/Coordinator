...
function MessageBubble({ message }: { message: ChatMessage }) {
  ...
  return (
    <div className={["flex", message.isMe ? "justify-end" : "justify-start"].join(" ")}>
      <div className={["max-w-[70%]", message.isMe ? "order-1" : "order-2"].join(" ")}>
        {/* 발신자 이름 (상대방 메시지만) */}
        {!message.isMe && (
        ...
        {/* 시간 */}
        <p className={["text-xs text-slate-400 mt-1 px-1", message.isMe ? "text-right" : "text-left"].join(" ")}>
          {time}
        </p>
      </div>
    </div>
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-slate-100">
        <div className="flex gap-2">
          <input
            ...
            placeholder={disabled ? "방에 먼저 참여해주세요" : "메시지를 입력하세요..."}
            disabled={disabled}

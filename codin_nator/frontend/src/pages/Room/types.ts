 ...
 * Room 모듈 - 타입 정의
 * ========================
 * WebRTC와 WebSocket을 사용하는 Room에서 필요한 모든 타입을 정의합니다.
 * 
 * 💡 학습 포인트:
 * - TypeScript의 Union Type을 사용하여 시그널링 메시지를 타입 안전하게 정의
 * - 서버와 클라이언트 간의 메시지 구조를 명확하게 정의
 */

// ============================================================
// 시그널링 메시지 타입 (WebSocket으로 주고받는 메시지)
// ============================================================
...
export type SignalMessage =
  ...
  // 텍스트 채팅 메시지
  | { type: "chat"; roomId: string; userId: string; userName: string; message: string; timestamp: number };
...
export interface UseWebRTCReturn {
  ...
  handleOffer: (peerId: string, sdp: RTCSessionDescriptionInit) => Promise<RTCSessionDescriptionInit | null>;
